  ...
  <q-drawer
    ...
    <div class="q-gutter-y-md q-pa-sm" v-if="todo">
        ...
        <q-card-section class="q-pa-none">
          <q-list separator flat>
            <q-item>
              <q-item-section side @click="$refs.qDateProxy.show()">
                <q-icon name="calendar_today" size="sm">
                  <q-popup-proxy ref="qDateProxy" transition-show="scale" transition-hide="scale">
                    <q-date v-model="date" @input="() => updateDueDate()" square mask="YYYY-MM-DD"/>
                  </q-popup-proxy>
                </q-icon>
              </q-item-section>
              ...
              <q-item-section
                v-else
                :class="{'text-primary': (!todo.is_overdue && todo.status !== 1), 'text-warning': todo.is_overdue}">
      ...
      <div class="flex justify-between items-center fixed-bottom q-pa-sm" :style="{borderTop: '1px solid #ccc'}">
        ...
        <q-btn unelevated dense icon="delete" color="negative" size="sm" @click="deleteTodo(todo)" />
      </div>
    </div>
  </q-drawer>
</template>
